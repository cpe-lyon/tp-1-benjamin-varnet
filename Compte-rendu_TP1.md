...
**Question 2 :** Quand on consulte une page du manuel, comment peut-on rechercher un terme (par exemple, chercher le terme option dans la page de manuel de `which` ? <br/>
...
Pour créer un dossier il faut faire mkdir "nom du dossier". Notre dossier sera donc créé. Puis, il faut faire un `cd /Notrenouveaudossier` ce qui va nous placer dans notre dossier créé précédemment, puis, il faut faire un `mkdir` dans ce dossier pour créé une sorte de sous-dossier.<br/>
...
**Question 15 :** Affichez à l’écran le fichier `/etc/passwd` ; que contient-il ? Quelle commande permet d’afficher la page de manuel de ce fichier ? `/etc/passwd` donne tous les comptes qui sont dans l'OS avec leur répertoire /home personnel.<br/>
`julien:x:1000:1000:julien:/home/julien:/bin/bash nom d'utilisateur:x` car c'était anciennement le mot de passe:id du user:id du groupe de user:groupe:répertoire de travail du user par défaut:intérpréteur de commande utilisé pour cet user<br/>
